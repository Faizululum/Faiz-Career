...
export const sendPeriodicJobListings = inngest.createFunction(
  ...
  async ({event, step}) => {
      ...
      if (recentJobs.length > 0) {
        await step.run("send-email", async () => {
          ...
          await resend.emails.send({
            from: "FaizCareer <faizululum25@gmail.com>",
            to: ["faizululum25@gmail.com"],
            subject: "Latest Job Opportunities - FaizCareer",
            html: `
    ...
    return { userId, message: "Completed 30 day job listing period notifications" };
  }
);